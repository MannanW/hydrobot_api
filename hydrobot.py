...
# ─────────────────────────────────────────────────────────────────────
DEFAULT_DATA_FILE = Path(__file__).resolve().parent / "data" / "Microgreens_dataa.xlsx"
ROOT_DATA_FILE = Path(__file__).resolve().parent / "Microgreens_dataa.xlsx"


def _resolve_data_file() -> str:
    data_file_env = os.environ.get("HYDROBOT_DATA_FILE", "")
    if data_file_env:
        return str(Path(data_file_env).expanduser())
...
def run_analysis(crop: str, scope: str, user_id: str) -> dict[str, Any]:
    ...
    X: NDArray[np.float64] = df[feature_cols].to_numpy(dtype=np.float64)
    y_w: NDArray[np.float64] = df[TARGET_WEIGHT].to_numpy(dtype=np.float64)
    y_h: NDArray[np.float64] = df[TARGET_HEIGHT].to_numpy(dtype=np.float64)

    X_train, X_test, yw_train, yw_test, yh_train, yh_test = train_test_split(
        X, y_w, y_h, test_size=0.2, random_state=42  # type: ignore[arg-type]
    )
